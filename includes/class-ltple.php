...
class LTPLE_Inbox {
	...
	public function __construct ( $file='', $parent, $version = '1.0.0' ) {
		...
		$this->parent->register_post_type( 'addon-post-type', __( 'Addon Type', 'live-template-editor-inbox' ), __( 'Addon Type', 'live-template-editor-inbox' ), '', array(
	...
	public function get_panel_url(){
		...
		$slug = get_option( $this->parent->_base . 'inboxSlug' );
		...
		if( empty( $slug ) ){
			...
			$slug = update_option( $this->parent->_base . 'inboxSlug', get_post($post_id)->post_name );
		}
